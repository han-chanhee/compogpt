...
with torch.no_grad():
    with ctx:
        while total_lines < 200:  # 텍스트가 200줄을 넘을 때까지 반복
            y = model.generate(x, max_new_tokens, temperature=temperature, top_k=top_k)
            generated_text = decode(y[0].tolist())

            # "[숫자]"를 텍스트에 삽입
            generated_text = generated_text.replace("[", f"[{sample_count}]")

            # 생성된 텍스트에 "["가 없을 때까지 반복해서 다시 생성
            while "[" in generated_text:
                y = model.generate(x, max_new_tokens, temperature=temperature, top_k=top_k)
                generated_text = decode(y[0].tolist())
                generated_text = generated_text.replace("[", f"[{sample_count}]")
            ...
            output_path = os.path.join(output_dir, f"sample_{sample_count}.txt")
            with open(output_path, "w") as file:
                file.write(generated_text)

            # 줄 수 증가 및 샘플 카운트 증가
            total_lines += generated_text.count('\n') + 1  # 줄 바꿈 문자('\n')을 기준으로 줄 수 계산
            sample_count += 1
